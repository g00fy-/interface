...
export function useENSName(account) {
  ...
  try {
    if (account && isAddress(account)) {
      ensNodeArgument = namehash(`${account.toLowerCase().substr(2)}.addr.reverse`)
    }
  } catch (error) {
    console.log(error)
  }
  ...
  useEffect(
    () => {
      if (registrarContract && ensNodeArgument) {
        try {
          registrarContract.resolver(ensNodeArgument)
  ...
  useEffect(
    () => {
      if (resolverContract && resolverAddress && !isZero(resolverAddress)) {
        try {
          resolverContract.name(ensNodeArgument)
            .then( fuck => setName(fuck) )
            .catch( e => console.log(e) )
        } catch (e) {
          console.log(e)
        }
